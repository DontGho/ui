...
do
	...
	do
		...
		Instances.Create = function(self, Class, Properties)
			...
			for Property, Value in NewItem.Properties do
				if Property == "TextSize" and (Class == "TextLabel" or Class == "TextButton" or Class == "TextBox") then
					Value = Library:GetTextSize(Value)
				end
		...
		Instances.Tooltip = function(self, Data)
			...
			Library:Connect(Gui.MouseEnter, function()
				...
				RenderStepped = RunService.RenderStepped:Connect(function()
					...
					Items["Tooltip"]:Tween(
						nil,
						{ Position = UDim2New(0, MouseLocation.X + 8, 0, MouseLocation.Y - 35) }
					)
				end)
			end)
	...
	Library.GetTextSize = function(self, Size)
		if type(Size) ~= "number" then
			return Size
		end

		return MathMax(Size, self.MinimumTextSize)
	end

	Library.BindInlineContent = function(self, ContainerItem, TextItem, TrailingItem, LeftOffset, RightOffset)
		if not ContainerItem or not TextItem or not TrailingItem then
			return function() end
		end

		local Container = ContainerItem.Instance
		local Text = TextItem.Instance
		local Trailing = TrailingItem.Instance

		LeftOffset = LeftOffset or 0
		RightOffset = RightOffset or 0

		local function Update()
			if not Container or not Text or not Trailing then
				return
			end

			local TrailingWidth = Trailing.AbsoluteSize.X
			local Spacing = TrailingWidth > 0 and 6 or 0
			local AvailableWidth =
				MathMax(Container.AbsoluteSize.X - LeftOffset - RightOffset - TrailingWidth - Spacing, 0)

			Text.Size = UDim2New(0, AvailableWidth, Text.Size.Y.Scale, Text.Size.Y.Offset)
			Trailing.AnchorPoint = Vector2New(1, 0.5)
			Trailing.Position = UDim2New(1, -RightOffset, 0.5, 0)
		end

		Update()

		self:Connect(Container:GetPropertyChangedSignal("AbsoluteSize"), Update)
		self:Connect(Trailing:GetPropertyChangedSignal("AbsoluteSize"), Update)
		self:Connect(Trailing:GetPropertyChangedSignal("Visible"), Update)
	...
	do
		Components.Window = function(self, Data)
			...
			do
				...
				if Data.Resizeable then
					Items["Window"]:MakeResizeable(
						Vector2New(Data.Size.X.Offset, Data.Size.Y.Offset),
		...
		Components.WindowPage = function(self, Data)
			...
			do
				...
				Items["GlowGradient"] = Instances:Create("UIGradient", {
					...
					Transparency = NumSequence({
						...
						NumSequenceKeypoint(0.08, 0.55),
						NumSequenceKeypoint(0.3, 0.88),
						NumSequenceKeypoint(0.6, 0.97),
						NumSequenceKeypoint(1, 1),
					}),
				})
				...
				Instances:Create("UIGradient", {
					...
					Transparency = NumSequence({
						...
						NumSequenceKeypoint(0.15, 0.92),
						NumSequenceKeypoint(0.5, 1),
						NumSequenceKeypoint(1, 1),
					}),
				})
			...
			function Page:Turn(Bool)
				...
				if Page.Active then
					...
					if Page.Window and Page.Window.ApplySearch then
						Page.Window:ApplySearch()
					end
				else
		...
		Components.WindowSubPage = function(self, Data)
			...
			do
				...
				Instances:Create("UIGradient", {
					...
					Transparency = NumSequence({
						NumSequenceKeypoint(0, 0),
						NumSequenceKeypoint(0.193, 0.8687499761581421),
						NumSequenceKeypoint(0.504, 0.96875),
						NumSequenceKeypoint(1, 1),
					}),
				})
			...
			function SubPage:Turn(Bool)
				...
				Debounce = true
				Items["Page"].Instance.Visible = Bool
				Items["Page"].Instance.Parent = Bool and Data.Page.Items["Columns"].Instance
				...
				if SubPage.Active then
					...
					if Data.Page.Window and Data.Page.Window.ApplySearch then
						Data.Page.Window:ApplySearch()
					end
				else
		...
		Components.Toggle = function(self, Data)
			...
			do
				...
				Instances:Create("UIGradient", {
					...
					Color = RGBSequence({
						...
						RGBSequenceKeypoint(1, FromRGB(208, 208, 208)),
					}),
				}):AddToTheme({
					Color = function()
						return RGBSequence({
				...
				UpdateInlineLayout =
					Library:BindInlineContent(Items["Toggle"], Items["Text"], Items["SubElements"], 26, 0)
			end
			...
			function Toggle:SetText(Text)
				...
				if UpdateInlineLayout then
					task.defer(UpdateInlineLayout)
				end
			end
			...
			function Toggle:Set(Value)
				...
				if Toggle.Value then
					...
					task.spawn(function()
						...
						TweenService
							:Create(pulseInst, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		...
		Components.Label = function(self, Data)
			...
			do
				...
				UpdateInlineLayout =
					Library:BindInlineContent(Items["Label"], Items["Text"], Items["SubElements"], 0, 0)
			end
			...
			function Dropdown:SetOpen(Bool)
				...
				if Dropdown.IsOpen then
					...
					RenderStepped = RunService.RenderStepped:Connect(function()
						optionHolder.Position = UDim2New(
							0,
							realDropdown.AbsolutePosition.X,
							0,
							realDropdown.AbsolutePosition.Y + realDropdown.AbsoluteSize.Y + 5
						)
					end)

					local targetSize = UDim2New(
						0,
						realDropdown.AbsoluteSize.X,
						0,
						optionHolder:FindFirstChildOfClass("UIListLayout").AbsoluteContentSize.Y + 10
					)
					TweenService:Create(
						optionHolder,
						TweenInfo.new(0.25, Enum.EasingStyle.Quart, Enum.EasingDirection.Out),
					...
					TweenService:Create(
						...
						{ Size = UDim2New(0, realDropdown.AbsoluteSize.X, 0, 0) }
					):Play()
				end
				...
				if NewTween and NewTween.Tween then
					NewTween.Tween.Completed:Connect(function()
						...
						task.wait(0.2)
						optionHolder.Parent = not Dropdown.IsOpen and Library.UnusedHolder.Instance
		...
		Components.Colorpicker = function(self, Data) -- poetry warning (╯°□°)╯
							...
							if KeyframeOneLabel and KeyframeTwoLabel and AnimationSpeedSlider then
								...
								Library:Thread(function()
									while task.wait() do
										...
										Colorpicker:Set(
											KeyframeOneColorpicker.Color:Lerp(FromRGB(0, 0, 0), Speed),
											Colorpicker.Alpha
										)
										UpdateSync(true)
			...
			if OtherTab then
				...
				Instances:Create("UIGradient", {
					...
					Color = RGBSequence({
						RGBSequenceKeypoint(0, FromRGB(255, 255, 255)),
						RGBSequenceKeypoint(1, FromRGB(154, 154, 154)),
					}),
				})
			...
			function Colorpicker:Update(IsFromAlpha, UpdateSyncc)
				...
				if OtherTab then
					...
					local FloorHue, FloorSat, FloorVal =
						Library:Round(Hue, 0.01), Library:Round(Saturation, 0.01), Library:Round(Value, 0.01)

					Items["RGBColor"].Instance.Text = "RGB: " .. Library:ToRich(RedGreenBlue, Colorpicker.Color)
					Items["HSVColor"].Instance.Text =
						`HSV: %{Library:ToRich(FloorHue, Colorpicker.Color)}, %{Library:ToRich(
							FloorSat,
							Colorpicker.Color
						)}, %{Library:ToRich(FloorVal, Colorpicker.Color)}`
					Items["HEXColor"].Instance.Text = "HEX: "
						.. "#"
						.. Library:ToRich(Colorpicker.HexValue, Colorpicker.Color)
				end
	...
	Library.Window = function(self, Data)
		...
		do
			...
			Instances:Create("UIGradient", {
				...
				Color = RGBSequence({
					RGBSequenceKeypoint(0, FromRGB(255, 255, 255)),
					RGBSequenceKeypoint(1, FromRGB(99, 108, 117)),
				}),
			})
		...
		function Window:SetOpen(Bool)
			...
			if Window.IsOpen then
				...
				TweenService:Create(
					winInst,
					TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.In),
					{ Size = UDim2New(0, finalSize.X.Offset * 0.95, 0, finalSize.Y.Offset * 0.95) }
				):Play()
			end
		...
		function Window:ApplySearch()
			...
			local Query = StringLower(Items["Input"].Instance.Text or "")
			...
			for _, Value in PageSearchData do
				...
				local Matches = Query == "" or StringFind(Name, Query, 1, true) ~= nil
				...
				if Matches then
					...
					local HiddenSize = UDim2New(
						OldSize and OldSize.X.Scale or Element.Instance.Size.X.Scale,
						OldSize and OldSize.X.Offset or Element.Instance.Size.X.Offset,
						0,
						0
					)

					local HideTween = Element:Tween(
						TweenInfo.new(0.08, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
						{ Size = HiddenSize }
					)
						...
						HideTween.Tween.Completed:Connect(function()
							if
								...
								and not (
									Query == ""
									or StringFind(
											StringLower(tostring(Value.Name or "")),
											StringLower(Items["Input"].Instance.Text or ""),
											1,
											true
										)
										~= nil
		...
		do
			...
			windowInst.Position =
				UDim2New(finalPos.X.Scale, finalPos.X.Offset, finalPos.Y.Scale, finalPos.Y.Offset - 30)
	...
	Library.Pages.Playerlist = function(self, Data)
			...
			Callback = function(Value)
				if Playerlist.Player then
					...
					if Value == "Neutral" then
						Playerlist.Players[Playerlist.Player.Name].PlayerStatus:Tween(
							nil,
		...
		function Playerlist:Add(Player)
			...
			local TeamColor = Player.TeamColor ~= nil and BrickColor.new(tostring(Player.TeamColor)).Color
				or Color3.new(1, 1, 1)
		...
		do
			...
			Instances:Create("UIGradient", {
				...
				Transparency = NumSequence({
					...
					NumSequenceKeypoint(0.15, 0.88),
					NumSequenceKeypoint(0.45, 0.97),
					NumSequenceKeypoint(1, 1),
				}),
			})
	...
	Library.CreateSettingsPage = function(self, Window, Watermark, KeybindList)
		...
		do
			local ThemingSubPage = SettingsPage:SubPage({ Name = "Theming", Columns = 2 })
			do
				local ThemesSection = ThemingSubPage:Section({ Name = "Themes", Side = 1 })
				do
					for Index, Value in Library.Theme do
						if Index == "BackgroundTransparency" then
							continue
						end
				...
				local AppearanceSection = ThemingSubPage:Section({ Name = "Appearance", Side = 2 })
				do
			...
			local ConfigsSubPage = SettingsPage:SubPage({ Name = "Configs", Columns = 2 })
			do
						...
						if ConfigName and ConfigName ~= "" then
							if not isfile(Library.Folders.Configs .. "/" .. ConfigName .. ".json") then
								...
								Library:Notification(
									"Error",
									"Config with the name " .. ConfigName .. " already exists",
									5
								)
								return
							end
			...
			local SettingsSubPage = SettingsPage:SubPage({ Name = "Settings", Columns = 2 })
			do
